export function AnalyzeStage({ filename, pageCount, busy, error, status, canCancel, onCancel, onRetry, onBack }: {
  filename: string;
  pageCount: number;
  busy: boolean;
  error: string | null;
  ...
  return (
    <section className="analyze-stage narrow-stage" aria-live="polite">
      ...
      {!error && canCancel && <button className="secondary-button" onClick={onCancel}>Continue without local analysis</button>}
